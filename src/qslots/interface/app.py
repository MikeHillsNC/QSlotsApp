...
from qslots.data.settings import Settings
...
class App:
    app = QApplication([])  # sys.args are passed automatically
    window = MainWindow()
    layout = QVBoxLayout()

    icon_path = resource_filename(
        'qslots',
        os.path.join('resources', 'qslots.svg')
    )
    ...
    def build(cls):
        ...
        cls.app.setApplicationName("qslots")
        cls.app.setApplicationVersion('2.1.0')
        cls.app.setDesktopFileName('com.codemouse92.qslots')
    ...
    def hide_window(cls):
        ...
        for callback in cls.hide_callback:
            callback("qslots is still running in the notification area.")
